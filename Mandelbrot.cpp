...
// --- New helper: apply the current selection to the same window (zoom/center) ---
static void ApplySelectionToWindow(AppState* s, HWND hwnd)
{
    ...
    double newScale = s->scale * ((double)selW / (double)s->width);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        ...
        case WM_MOUSEMOVE:
        {
            ...
            // --- Replace the selection-sizing logic inside WM_MOUSEMOVE (selecting branch) ---
            // (This snippet replaces the original branch where selRect was simply the start->current mouse)
            else if (s->selecting)
            {
                int x = LOWORD(lParam);
                int y = HIWORD(lParam);
